...
async def run2(
    weights=ROOT / "yolov5s.pt",  # model path or triton URL
    source="http://192.168.2.148:8080/?action=stream",  # file/dir/URL/glob/screen/0(webcam)
    data=ROOT / "data/coco128.yaml",  # dataset.yaml path
    imgsz=(640, 640),  # inference size (height, width)
    conf_thres=0.25,  # confidence threshold
    iou_thres=0.45,  # NMS IOU threshold
    ...
    if save_txt or save_img:
        ...
        LOGGER.info(f"Results saved to {colorstr('bold', save_dir)}{s}")
    if update:
        strip_optimizer(weights[0])  # update model (to fix SourceChangeWarni
...
async def inference1():
    asyncio.create_task(run(source="http://192.168.1.3:4747/video"))
